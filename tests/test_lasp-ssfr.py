...
def figure_ingaas_dark_counts_time_series(
    ...
    f = h5py.File('nonlin_cal.h5', 'r')

    # SSFR-A
    #/----------------------------------------------------------------------------\#
    wvls = f['ssfr-a_%s/wvl_%s' % (which_lc.lower(), which_spec.lower())][...]
    index_chan = np.argmin(np.abs(wvls-wvl0))

    shutter0 = f['ssfr-a_%s/shutter' % (which_lc.lower())][...]
    int_time_in0 = f['ssfr-a_%s/int_time' % (which_lc.lower())][...][:, index_spec]
    logic_time = (shutter0==1)&(int_time_in0==int_time0)

    dark_cnt_in_sky_a0 = f['ssfr-a_%s/counts' % (which_lc.lower())][...][logic_time, index_chan, index_spec]
    temp_a0 = f['ssfr-a_%s/temperature' % (which_lc.lower())][...][logic_time, index_temp]
    #\----------------------------------------------------------------------------/#

    # SSFR-B
    #/----------------------------------------------------------------------------\#
    wvls = f['ssfr-b_%s/wvl_%s' % (which_lc.lower(), which_spec.lower())][...]
    index_chan = np.argmin(np.abs(wvls-wvl0))

    shutter0 = f['ssfr-b_%s/shutter' % (which_lc.lower())][...]
    int_time_in0 = f['ssfr-b_%s/int_time' % (which_lc.lower())][...][:, index_spec]
    logic_time = (shutter0==1)&(int_time_in0==int_time0)

    dark_cnt_in_sky_b0 = f['ssfr-b_%s/counts' % (which_lc.lower())][...][logic_time, index_chan, index_spec]
    temp_b0 = f['ssfr-b_%s/temperature' % (which_lc.lower())][...][logic_time, index_temp]
    #\----------------------------------------------------------------------------/#
    ...
    if True:
        ...
        patches_legend = [
                         mpatches.Patch(color='blue', label='SSFR-A'), \
                         mpatches.Patch(color='red' , label='SSFR-B'), \
                         ]
        ax2.legend(handles=patches_legend, loc='lower right', fontsize=16)
        #\--------------------------------------------------------------/#
