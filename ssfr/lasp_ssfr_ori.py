...
class read_ssfr:
    ...
    def dark_corr(
        ...
        for idset in range(self.Ndset):
            ...
            spectra_dark_offset = np.zeros((256, self.Nspec))
            spectra_dark_offset[...] = fill_value
            spectra_dark_offset_std = spectra_dark_offset.copy()
            print('self.Nspec:',self.Nspec)
            for ip in range(self.Nspec):
                shutter_dark_corr, spectra_dark_corr[:, :, ip], spectra_dark_offset[:, ip], spectra_dark_offset_std[:, ip]  = ssfr.corr.dark_corr(dset['tmhr'], dset['shutter'], dset['spectra'][:, :, ip], mode=dark_corr_mode, darkExtend=dark_extend, lightExtend=light_extend, fillValue=fill_value)

            dset['shutter_dark-corr'] = shutter_dark_corr
            dset['spectra_dark-corr'] = spectra_dark_corr
            dset['spectra_dark_offset'] = spectra_dark_offset
            dset['spectra_dark_offset_std'] = spectra_dark_offset_std
            #\----------------------------------------------------------------------------/#
        #\----------------------------------------------------------------------------/#
