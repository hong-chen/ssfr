...
def pre_bokeh_spns(fname, tmhr0=None, wvl0=None, tmhr_range=None, tmhr_step=2, wvl_step=2):
    ...
    if wvl0 is None:
        wvl0 = 500.0
    index_wvl = np.argmin(np.abs(data['spns_wvl']-wvl0))

    data_time = {}
    data_time['tmhr'] = data['tmhr']
    data_time['jday'] = data['jday']

    data_time['dif_plot'] = data['dif_flux'][:, index_wvl]
    data_time['tot_plot'] = data['tot_flux'][:, index_wvl]
    for i in range(data['spns_wvl'].size):
        data_time['dif%d' % i] = data['dif_flux'][:, i]
        data_time['tot%d' % i] = data['tot_flux'][:, i]

    data_time['mu'] = data['mu']
    data_time['sol_plot'] = data['solar'][index_wvl] * data['mu']

    data_time['alt']  = data['alt'] / (np.nanmax(data['alt'])-np.nanmin(data['alt']))\
            * (np.nanmax(data['tot_flux'][:, index_wvl])-np.nanmin(data['tot_flux'][:, index_wvl]))
    #\----------------------------------------------------------------------------/#
    ...
    #/----------------------------------------------------------------------------\#
    if tmhr0 is None:
        index_tmhr = np.where(np.logical_not(np.isnan(data['tot_flux'][:, index_wvl])))[0][0]
    else:
        index_tmhr = np.argmin(np.abs(data['tmhr']-tmhr0))
    ...
    data_spns['solar']    = data['solar']
    data_spns['sol_plot'] = data['solar'] * data['mu'][index_tmhr]
    #\----------------------------------------------------------------------------/#
    ...
    return data_time, data_spns, data_geo
